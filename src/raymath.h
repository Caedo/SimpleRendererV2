...
// - changed Matrix definition to be Column-Major in memory.
// - changed Matrix elements names
// - fixed weird matrix multiplication (it appeard to be backwards?)
// - added function and operator overloadings
...
RMAPI Vector2 Vector2Transform(Vector2 v, Matrix mat)
{
    ...
    result.x = mat.m00*x + mat.m10*y + mat.m20*z + mat.m30;
    result.y = mat.m01*x + mat.m11*y + mat.m21*z + mat.m31;
...
RMAPI Vector3 Vector3Transform(Vector3 v, Matrix mat)
{
    ...
    result.x = mat.m00*x + mat.m10*y + mat.m20*z + mat.m30;
    result.y = mat.m01*x + mat.m11*y + mat.m21*z + mat.m31;
    result.z = mat.m02*x + mat.m12*y + mat.m22*z + mat.m32;
...
RMAPI Vector3 Vector3Unproject(Vector3 source, Matrix projection, Matrix view)
{
    ...
    Matrix matViewProj = {      // MatrixMultiply(view, projection);
        view.m00*projection.m00 + view.m01*projection.m10 + view.m02*projection.m20 + view.m03*projection.m30,
        view.m00*projection.m01 + view.m01*projection.m11 + view.m02*projection.m21 + view.m03*projection.m31,
        view.m00*projection.m02 + view.m01*projection.m12 + view.m02*projection.m22 + view.m03*projection.m32,
        view.m00*projection.m03 + view.m01*projection.m13 + view.m02*projection.m23 + view.m03*projection.m33,
        view.m10*projection.m00 + view.m11*projection.m10 + view.m12*projection.m20 + view.m13*projection.m30,
        view.m10*projection.m01 + view.m11*projection.m11 + view.m12*projection.m21 + view.m13*projection.m31,
        view.m10*projection.m02 + view.m11*projection.m12 + view.m12*projection.m22 + view.m13*projection.m32,
        view.m10*projection.m03 + view.m11*projection.m13 + view.m12*projection.m23 + view.m13*projection.m33,
        view.m20*projection.m00 + view.m21*projection.m10 + view.m22*projection.m20 + view.m23*projection.m30,
        view.m20*projection.m01 + view.m21*projection.m11 + view.m22*projection.m21 + view.m23*projection.m31,
        view.m20*projection.m02 + view.m21*projection.m12 + view.m22*projection.m22 + view.m23*projection.m32,
        view.m20*projection.m03 + view.m21*projection.m13 + view.m22*projection.m23 + view.m23*projection.m33,
        view.m30*projection.m00 + view.m31*projection.m10 + view.m32*projection.m20 + view.m33*projection.m30,
        view.m30*projection.m01 + view.m31*projection.m11 + view.m32*projection.m21 + view.m33*projection.m31,
        view.m30*projection.m02 + view.m31*projection.m12 + view.m32*projection.m22 + view.m33*projection.m32,
        view.m30*projection.m03 + view.m31*projection.m13 + view.m32*projection.m23 + view.m33*projection.m33 };

    // Calculate inverted matrix -> MatrixInvert(matViewProj);
    // Cache the matrix values (speed optimization)
    float a00 = matViewProj.m00, a01 = matViewProj.m01, a02 = matViewProj.m02, a03 = matViewProj.m03;
    float a10 = matViewProj.m10, a11 = matViewProj.m11, a12 = matViewProj.m12, a13 = matViewProj.m13;
    float a20 = matViewProj.m20, a21 = matViewProj.m21, a22 = matViewProj.m22, a23 = matViewProj.m23;
    float a30 = matViewProj.m30, a31 = matViewProj.m31, a32 = matViewProj.m32, a33 = matViewProj.m33;
    ...
    Quaternion qtransformed = {     // QuaternionTransform(quat, matViewProjInv)
        matViewProjInv.m00*quat.x + matViewProjInv.m10*quat.y + matViewProjInv.m20*quat.z + matViewProjInv.m30*quat.w,
        matViewProjInv.m01*quat.x + matViewProjInv.m11*quat.y + matViewProjInv.m21*quat.z + matViewProjInv.m31*quat.w,
        matViewProjInv.m02*quat.x + matViewProjInv.m12*quat.y + matViewProjInv.m22*quat.z + matViewProjInv.m32*quat.w,
        matViewProjInv.m03*quat.x + matViewProjInv.m13*quat.y + matViewProjInv.m23*quat.z + matViewProjInv.m33*quat.w };
...
{
    float result = (mat.m00 + mat.m11 + mat.m22 + mat.m33);
...
{
    Matrix result = { 1.0f, 0.0f, 0.0f, 0,
                      0.0f, 1.0f, 0.0f, 0,
                      0.0f, 0.0f, 1.0f, 0,
                      x, y, z, 1.0f };
...
RMAPI Matrix MatrixRotateXYZ(Vector3 angle)
{
    ...
    result.m01 = (cosz*siny*sinx) - (sinz*cosx);
    result.m02 = (cosz*siny*cosx) + (sinz*sinx);

    result.m10 = sinz*cosy;
    result.m11 = (sinz*siny*sinx) + (cosz*cosx);
    result.m12 = (sinz*siny*cosx) - (cosz*sinx);
...
RMAPI Matrix MatrixFrustum(double left, double right, double bottom, double top, double near, double far)
{
    ...
    result.m20 = ((float)right + (float)left)/rl;
    result.m21 = ((float)top + (float)bottom)/tb;
    result.m22 = -((float)far + (float)near)/fn;
    result.m23 = -1.0f;

    result.m30 = 0.0f;
    result.m31 = 0.0f;
    result.m32 = -((float)far*(float)near*2.0f)/fn;
    result.m33 = 0.0f;
...
RMAPI Matrix MatrixOrtho(double left, double right, double bottom, double top, double near, double far)
{
    ...
    result.m22 = -2.0f/fn;
    result.m23 = 0.0f;
    result.m30 = -((float)left + (float)right)/rl;
    result.m31 = -((float)top + (float)bottom)/tb;
    result.m32 = -((float)far + (float)near)/fn;
    result.m33 = 1.0f;
...
RMAPI Matrix MatrixLookAt(Vector3 eye, Vector3 target, Vector3 up)
{
    ...
    result.m22 = vz.z;
    result.m23 = 0.0f;
    result.m30 = -(vx.x*eye.x + vx.y*eye.y + vx.z*eye.z);   // Vector3DotProduct(vx, eye)
    result.m31 = -(vy.x*eye.x + vy.y*eye.y + vy.z*eye.z);   // Vector3DotProduct(vy, eye)
    result.m32 = -(vz.x*eye.x + vz.y*eye.y + vz.z*eye.z);   // Vector3DotProduct(vz, eye)
    result.m33 = 1.0f;
...
RMAPI Quaternion QuaternionFromMatrix(Matrix mat)
{
    ...
    float fourWSquaredMinus1 = mat.m00 + mat.m11 + mat.m22;
    float fourXSquaredMinus1 = mat.m00 - mat.m11 - mat.m22;
    float fourYSquaredMinus1 = mat.m11 - mat.m00 - mat.m22;
    float fourZSquaredMinus1 = mat.m22 - mat.m00 - mat.m11;
    ...
    switch (biggestIndex)
    {
        case 0:
            ...
            result.y = (mat.m20 - mat.m02) * mult;
            result.z = (mat.m01 - mat.m10) * mult;
            break;
        case 1:
            result.x = biggestVal;
            result.w = (mat.m12 - mat.m21) * mult;
            result.y = (mat.m01 + mat.m10) * mult;
            result.z = (mat.m20 + mat.m02) * mult;
            break;
        case 2:
            result.y = biggestVal;
            result.w = (mat.m20 - mat.m02) * mult;
            result.x = (mat.m01 + mat.m10) * mult;
            result.z = (mat.m12 + mat.m21) * mult;
            break;
        case 3:
            result.z = biggestVal;
            result.w = (mat.m01 - mat.m10) * mult;
            result.x = (mat.m20 + mat.m02) * mult;
...
RMAPI Quaternion QuaternionTransform(Quaternion q, Matrix mat)
{
    ...
    result.x = mat.m00*q.x + mat.m10*q.y + mat.m20*q.z + mat.m30*q.w;
    result.y = mat.m01*q.x + mat.m11*q.y + mat.m21*q.z + mat.m31*q.w;
    result.z = mat.m02*q.x + mat.m12*q.y + mat.m22*q.z + mat.m32*q.w;
    result.w = mat.m03*q.x + mat.m13*q.y + mat.m23*q.z + mat.m33*q.w;
...
RMAPI Vector4 operator*(Matrix mat, Vector4 v) {
    Vector4 result = { 0 };
    ...
    result.x = mat.m00*x + mat.m10*y + mat.m20*z + mat.m30*w;
    result.y = mat.m01*x + mat.m11*y + mat.m21*z + mat.m31*w;
    result.z = mat.m02*x + mat.m12*y + mat.m22*z + mat.m32*w;
    result.w = mat.m03*x + mat.m13*y + mat.m23*z + mat.m33*w;
